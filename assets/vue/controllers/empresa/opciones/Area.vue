<template>
    <div class="container">
        <div class="area">
            ...
            <!-- Línea azul con poco espacio inferior -->
            ...
            <div class="row mb-4">
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        >
                            ...
                            <option v-for="(area, index) in storeAreas" :key="index" :value="index">{{ area.nombre }}</option>
                        </select>
                    </div>
                ...
                <div class="col-md-6 d-flex justify-content-center align-items-center">
                    <div class="border p-3" style="width: 100%; height: 150px; overflow-y: auto;">
                        <!-- Mostrar puestos del área seleccionada -->
                        ...
                        <p v-for="(puesto, index) in selectedAreaPuestos" :key="index" class="mb-1">
                            {{ puesto.nombre }}
                        </p>
                    </div>
                </div>
            ...
            <div v-if="store.registroExitoso" class="alert alert-success mt-3">{{ store.registroExitoso }}</div>
            <div v-if="store.errorRegistro" class="alert alert-danger mt-3">{{ store.errorRegistro }}</div>
        </div>
    </div>
</template>
...
const areaSeleccionada = ref('');  // Área seleccionada para el puesto
...
const selectedAreaPuestos = computed(() => {
    ...
    return storeAreas.value[areaSeleccionada.value]?.puestos || [];
});
...
const guardarPuesto = () => {
    if (areaSeleccionada.value && nuevoPuesto.value) {
        const area = storeAreas.value[areaSeleccionada.value];
        if (area) {
            area.puestos.push({ nombre: nuevoPuesto.value });
...
const guardarEmpresa = () => {
    store.registrarAreas(storeAreas.value);
};
</script>
